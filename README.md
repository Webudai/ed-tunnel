...
2. Alternatively, you can click the button below to deploy directly.
   ...
   [![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/seramo/ed-tunnel)
...
1. When deploy in cloudflare pages, you can set uuid in `wrangler.toml` file. variable name is `UUID`. `wrangler.toml` file is also supported. (recommended) in case dep# EDtunnel
...
loy in webpages, you can not set uuid in `wrangler.toml` file.
